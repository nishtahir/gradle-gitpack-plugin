...
open class FetchDependenciesTask : DefaultTask() {
    ...
    val localMavenRepository by lazy { "$project/.maven" }
    ...
    fun fetchDependencies() {
        ...
        gitConfiguration.dependencies.forEach { dependency ->
            ...
            val artifact: File = when {
                dependentProjectRoot.isMavenProject() -> {
                    buildMavenProject(dependentProjectRoot)
                    getArtifactFromTarget(File(dependentProjectRoot, "target"))
                }
                dependentProjectRoot.isGradleProject() -> {
    ...
    fun openOrCloneRepository(directory: File, pathToRepository: String) = try {
        ...
        val git = if (repositoryBuilder.gitDir == null) {
            ...
            Git(repositoryBuilder.build()).apply { pull().call() }
        }
        git.close()
    } catch (exception: Exception) {
    ...
    fun getArtifactFromTarget(buildDirectory: File): File {
        val artifact = buildDirectory.listFiles().find { it.extension == "jar" }
    ...
    fun File.isMavenProject(): Boolean {
        return this.isDirectory && File(this, "pom.xml").exists()
    }

    fun File.isGradleProject(): Boolean {
        return this.isDirectory && File(this, "build.gradle").exists()
    }
}
